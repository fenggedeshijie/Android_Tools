...
public class SlideCutListView extends ListView {
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {  
        ...
        case MotionEvent.ACTION_MOVE: {  
            ...
            if (getScrollVelocity() > SNAP_VELOCITY  	// 滑动启动的条件
                    || event.getX() - downX > mTouchSlop && Math.abs(event.getY() - downY)
                    <= (event.getX() - downX) * Math.tan(Math.toRadians(30))) {  
                isSlide = true;
            }  
            break;  
        }  
    ...
    public boolean onTouchEvent(MotionEvent ev) {  
        if (isSlide && slidePosition != AdapterView.INVALID_POSITION) {  
            ...
            case MotionEvent.ACTION_MOVE:   
                ...
                float scale = 2.0f / screenWidth;
                float alpha = 1.0f - scale * Math.abs(itemView.getScrollX());
    ...
    private void scrollRight() {  
        ...
        postInvalidate(); // 此时需要手动刷新View 否则没效果
    }  
    ...
    private void scrollLeft() {  
        removeDirection = RemoveDirection.START;  
        final int delta = (screenWidth - itemView.getScrollX());  
        // 调用startScroll方法来设置一些滚动的参数，我们在computeScroll()方法中调用scrollTo来滚动item  
        mScroller.startScroll(itemView.getScrollX(), 0, delta, 0, Math.abs(delta));  
    ...
    private void scrollBack() {
    	 ...
    	 final int delta = itemView.getScrollX(); 
         mScroller.startScroll(itemView.getScrollX(), 0, -delta, 0, 2*Math.abs(delta));  
         postInvalidate(); // 刷新itemView 
    }
